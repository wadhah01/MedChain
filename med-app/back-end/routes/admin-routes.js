...
exports.createPatient = async (req, res) => {
  ...
  /*Generally we create patient id by ourselvs so if patient id is not present in the request then fetch last id
  ...
  if (!('patientId' in req.body) || req.body.patientId === null || req.body.patientId === '') {
    ...
    req.body.patientId = 'PID' + (parseInt(lastId.slice(3)) + 2);
  }
...
/* Body must be a doctor json and role in the header
 res 201 response if asset is created else 400 with a simple json message
  Creates a doctor as an user adds the doctor to the wallet
 */
exports.createDoctor = async (req, res) => {
  ...
  if (response.error) {
    ...
    res.status(201).send(`successfuly registered doctor ${username}`);
  }
  
};

/*
 req Role in the header
 res 200 response with the json of all the assets(patients) in the ledger
