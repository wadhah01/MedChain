 ...
 const channelName = 'channel6';
 const chaincodeName = 'medical-v6';
 ...
 exports.connectToNetwork = async function(Identity) {
   const gateway = new Gateway();
   const ccp = buildCCPHosp1();
 
   try {
     const walletPath = path.join(process.cwd(), '..','javascript-application','wallet','/');
 
     const wallet = await buildWallet(Wallets, walletPath);
     
     const userExists = await wallet.get(Identity);
     if (!userExists) {
       console.log('An identity for the doctorID: ' + Identity + ' does not exist in the wallet');
       console.log('Create the doctorID before retrying');
       const response = {};
       response.error = 'An identity for the user ' + Identity + ' does not exist in the wallet. Register ' + Identity + ' first';
       return response;
     }
     ...
     */
     ...
     // using Localhost as thie gateway to the fabric network deployed locally
 ...
 /*
   ...
   isQuery true if retieving from ledger, else false in the case of add a transaction to the ledger.

   func must be the function name in the chaincode.
   
  args - a json string, if there are mutiple args, the args must be a json as one string
  *return stringresponse if the transaction was successful
  return  response error otherwise
  A common function to interact with the ledger
  */
 ...
 exports.registerUser = async function(attributes) {
   ...
   try {
     ...
     if (hospitalId === 1) {
       ...
       await registerAndEnrollUser(caClient, wallet, mspOrg1, userId, 'admin1', attributes);
     } else if (hospitalId === 2) {
       const ccp = buildCCPHosp2();
       const caClient = buildCAClient(FabricCAServices, ccp, 'ca.org2.example.com');
       await registerAndEnrollUser(caClient, wallet, mspOrg2, userId, 'admin2', attributes);
     } 
 ...
 /**
   networkObj:The object which is generated when connectToNetwork is executed
  hospitalId :Returns an JSON array consisting of all doctor objects.
   Retrieves all the users(doctors) based on user type(doctor) and hospitalId
  */
